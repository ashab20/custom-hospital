...
if(isset($_GET['time'])){
    ...
    if(isset($_GET['phone'])){
      $phone = $_GET['phone'];
      $patientData = $mysqli->custome_query("select id from patient where patient.phone='$phone'");
    ...
    $value=$Doctordata['selectdata'];
      ...
      $fees = $d["visit_fee"];
      $limit = $d["daily_approval_patient"];
      ...
      $value = ["status"=>"success","fees"=>floatval($fees),"time"=> (string) $time,"discount" => floatval($discount),"total"=>floatval($total),"limit"=>$limit  ];
    }else{
      $value =["msg"=>"Appointment is Booking Full","status"=>"faild"];
    }
